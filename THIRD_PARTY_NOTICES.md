...
The `iron-proxy` binary is embedded (gzipped) inside the devm binary
via `//go:embed` at `internal/ironproxy/embed/iron-proxy.gz`. The daemon
...
```
   ...
   Copyright 2025 The iron-proxy Authors
