"""100: the isolated-mode `restart_isolated_daemon` fixture seam works.

Task 7 deliverable proof — Task 8's heal e2e tests depend on this
fixture to reach the "adopted, dead-proxy-stays-dead" state (a
verified spike found the supervisor does NOT auto-restart an *adopted*
proxy, only a freshly-spawned one). Before those tests can rely on it,
this pins that:

  - `restart_isolated_daemon()` actually kills and relaunches the
    isolated daemon (new PID).
  ...
  - The project's iron-proxy is untouched by the restart — same PID
    ...
    respawned (mirrors the install-mode pin in test_44, but via the
    isolated seam instead of `devm service restart`).
"""
...
def test_restart_isolated_daemon_seam(devm, workspace, sandbox_name, devm_installed, restart_isolated_daemon):
    ...
    # The seam under test: kill + relaunch the isolated daemon in place.
    ...
    assert pid_after == pid_before, (
        f"iron-proxy PID changed across restart_isolated_daemon(): "
