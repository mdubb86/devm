...
describe('api', () => {
  ...
  it('getStatusAll parses JSON array', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith('devm-api://vm/status/all');
  });
  ...
  it('getVersion parses Build', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith('devm-api://vm/version');
  });
  ...
  it('getHandshake omits query param when no name given', async () => {
    ...
    await getHandshake();
    expect(fetch).toHaveBeenCalledWith('devm-api://vm/handshake');
  });
  ...
  it('getHandshake passes name query', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith('devm-api://vm/handshake?name=proj');
  });
  ...
  it('getHandshake URL-encodes the name query param', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith('devm-api://vm/handshake?name=proj%20name');
  });
