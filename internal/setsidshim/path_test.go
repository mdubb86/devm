...
// file → Ensure creates <runtimeDir>/bin/devm-setsid-shim from the
// embedded gzip blob, chmods it executable, and drops a matching
// sha256 sidecar.
func TestEnsure_FreshExtracts(t *testing.T) {
	...
	assert.Equal(t, filepath.Join(runDir, "bin", "devm-setsid-shim"), got)

	info, err := os.Stat(got)
	require.NoError(t, err)
	assert.NotZero(t, info.Size(), "extracted devm-setsid-shim must not be empty")
...
func TestEnsure_RewritesOnSidecarMismatch(t *testing.T) {
	...
	// Simulate an older devm having extracted a different
	// devm-setsid-shim — stale sidecar hash but a real (though wrong)
	// binary at the target.
	...
	assert.Greater(t, info.Size(), int64(len("stale-binary")),
		"target must be replaced with the real devm-setsid-shim, not left as the stale placeholder")
}

// TestEmbeddedSha256NonEmpty confirms the exported accessor returns the
// compiled-in hex sha256 of the embedded blob.
