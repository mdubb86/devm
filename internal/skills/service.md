...
Secrets live in the on-disk store at `~/Library/Application Support/devm/secrets/<project>/<name>`. At `devm start`/`devm reconcile` time, the CLI process (running as your user) reads each `!secret` ref, resolves it against the store, and passes the resolved values to the daemon as proxy-tokens. The daemon itself also reads the store directly when it needs to rebuild a project's iron-proxy config on its own — respawning after an unexpected iron-proxy death, or converging a live `packages:` change — so secret-injecting projects heal the same as any other. See `devm skills get secrets` for how secrets are declared and handed off.
