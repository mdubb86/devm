	...
	cases := []struct {
	...
	}{
		{
			...
			denylist: map[string]string{
				...
				"Memory":     "VM sizing is project-wide in this release; per-dev override via devm.me.yaml is a separate followup. Same class as ConfigLock — parity harness doesn't yet handle Ptr-typed base fields.",
				"Cpu":        "VM sizing is project-wide in this release; per-dev override via devm.me.yaml is a separate followup. Same class as ConfigLock — parity harness doesn't yet handle Ptr-typed base fields.",
				...
				"Masks":      "not overridable per environment; masks are project-wide platform-content-isolation declarations (docs/superpowers/specs/2026-08-02-masks-top-level-live-design.md) — no per-dev variance",
			},
		},
		{
			name:     "Project",
