...
// SystemKeychain is exported so cmd/devm/service.go can build the
...
// CheckCATrusted returns true if a cert with cfg.CACommonName() is
// present in the System Keychain. False with nil err means "not
// present"; a non-nil err means the `security` command failed.
//
// Reading the System Keychain doesn't require sudo — it's
// world-readable.
func CheckCATrusted(cfg identity.Config) (bool, error) {
	cmd := exec.Command("security", "find-certificate",
		"-c", cfg.CACommonName(), SystemKeychain)
