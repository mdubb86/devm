...
func TestIronProxyCheck_DriftMissing_RespawnedAndCacheUpdated(t *testing.T) {
	...
	fake := &fakeGroundTruth{
		...
		IronProxyHealthFn: func(ctx context.Context, projectID string) ProxyHealth {
			calls++
			if calls == 1 {
				return ProxyHealth{Status: ProxyMissing}
			}
			return ProxyHealth{Status: ProxyOK}
		},
	...
	row, _ := cache.ProjectRow("p")
	assert.Equal(t, ProxyOK, row.IronProxyHealth.Status)
}
...
func TestIronProxyCheck_DriftRepairFailed_CacheReflectsObservedAndErrors(t *testing.T) {
	...
	require.Error(t, err)
	row, _ := cache.ProjectRow("p")
	assert.Equal(t, ProxyMissing, row.IronProxyHealth.Status,
