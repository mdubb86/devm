...
// devm.yaml, devm.me.yaml, devm.sh, and devm.me.sh: the "*" blanket
// ignore covers the rest of the project's working tree, while the
// negations un-ignore only the four config files meant to cross into
// the guest.
var configSyncIgnores = []string{"*", "!devm.yaml", "!devm.me.yaml", "!devm.sh", "!devm.me.sh"}

// ConfigSyncSessionName returns the mutagen sync session name for
// projectName's dedicated config-file sync.
...
// session syncing <macCwd>/{devm.yaml,devm.me.yaml,devm.sh,devm.me.sh}
// with /home/devm/{devm.yaml,devm.me.yaml,devm.sh,devm.me.sh}. A no-op
// if the session already exists — this session is never
// paused/resumed like the per-entity workspace sessions, only created
// and (on stop) terminated outright, so "already exists" is the only
// warm-attach case there is.
